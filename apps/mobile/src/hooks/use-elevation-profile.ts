...
// Port **verbatim** du web `apps/web/src/hooks/use-elevation-profile.ts` (MOB-5.5 / T1).
// Pur, memoïsé, zéro DOM → 100 % portable. Entrées : `waypoints` (km cumulés à l'échelle
// de l'aventure, via `useAdventureWaypoints`) + `segments`. Sortie : la donnée du profil.
//
// **Memoïsation stricte (NFR-LP-002)** : `points[]` ne dépend QUE de `waypoints`/`segments`,
// JAMAIS du domaine visible / du slider. Le zoom = re-cadrage du domaine X côté rendu
// (`ElevationChart`), pas un re-tranchage des points → la référence `points` reste stable
// quand l'utilisateur bouge le slider (pas de re-render coûteux du `<Path>`).
  ...
  return useMemo(() => {
    ...
    for (let i = 0; i < validWaypoints.length; i++) {
      ...
      if (i > 0) {
        ...
        const deltaM =
          (validWaypoints[i].distKm - validWaypoints[i - 1].distKm) * 1000;
