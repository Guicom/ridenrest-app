...
describe('SearchOnDropdown', () => {
  ...
  it('shows both Booking (coord fallback) and Airbnb when no city but center available', () => {
    ...
    expect(link.href).toContain('/go/booking?url=')
    const encodedUrl = decodeURIComponent(link.href.split('url=')[1]!)
    expect(encodedUrl).toContain('dest_type=latlong')
  })

  it('Booking.com link uses proxy with ?ss=city when city provided', () => {
    render(<SearchOnDropdown center={center} city="Pamplona" />)
    fireEvent.click(screen.getByTestId('search-on-trigger'))
    const link = screen.getByTestId('search-on-booking') as HTMLAnchorElement
    expect(link.href).toContain('/go/booking?url=')
    const encodedUrl = decodeURIComponent(link.href.split('url=')[1]!)
    expect(encodedUrl).toContain('ss=Pamplona')
    expect(encodedUrl).not.toContain('dest_type')
  ...
  it('Booking.com uses coord fallback when no city but center available', () => {
    ...
    const link = screen.getByTestId('search-on-booking') as HTMLAnchorElement
    const encodedUrl = decodeURIComponent(link.href.split('url=')[1]!)
    expect(encodedUrl).toContain('latitude=43.5')
    expect(encodedUrl).toContain('longitude=1.4')
    expect(encodedUrl).not.toContain('ss=')
  })
  ...
  it('Booking.com link includes postcode in ss param when both city and postcode provided', () => {
    ...
    const link = screen.getByTestId('search-on-booking') as HTMLAnchorElement
    const encodedUrl = decodeURIComponent(link.href.split('url=')[1]!)
    expect(encodedUrl).toContain('ss=Saint-Jean-de-Luz%2064500')
  })
  ...
  it('Booking.com link includes adminArea and country in ss param (Story 16.31)', () => {
    ...
    const link = screen.getByTestId('search-on-booking') as HTMLAnchorElement
    const encodedUrl = decodeURIComponent(link.href.split('url=')[1]!)
    expect(encodedUrl).toContain('ss=Valencia%2046001%2C%20Comunidad%20Valenciana%2C%20Espa')
  })
  ...
  it('Booking.com link omits null adminArea/country gracefully', () => {
    ...
    const link = screen.getByTestId('search-on-booking') as HTMLAnchorElement
    const encodedUrl = decodeURIComponent(link.href.split('url=')[1]!)
    expect(encodedUrl).toContain('ss=Toulouse%2031000')
    expect(encodedUrl).not.toContain('%2C')
  })
