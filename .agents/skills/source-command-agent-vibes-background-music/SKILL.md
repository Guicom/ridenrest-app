...
Use this skill when the user asks to run the migrated source command `agent-vibes-background-music`.

## Command Template

**IMPORTANT: When user requests background music changes in natural language (e.g., "change to salsa", "switch to jazz"), you MUST:**

1. Run `.Codex/hooks/background-music-manager.sh list` to see all available tracks
2. Find the matching track by searching for keywords (e.g., "salsa" → "agent_vibes_salsa_v2_loop.mp3")
3. Execute `.Codex/hooks/background-music-manager.sh set-default EXACT_FILENAME`
4. Test with TTS to confirm the change took effect

**CRITICAL PATH INFORMATION:**
- The script is ALWAYS at `.Codex/hooks/background-music-manager.sh` (relative to current directory)
...
- Use either `bash .Codex/hooks/background-music-manager.sh` OR just `.Codex/hooks/background-music-manager.sh`
...
`bash .Codex/hooks/background-music-manager.sh {command}`
...
Background tracks are stored in `.Codex/audio/tracks/`

Agent-specific themes are configured in `.Codex/config/audio-effects.cfg`:
```
...
- `/agent-vibes:verbosity` - Control how much Codex speaks
