...
**What this does:**
- Moves all AgentVibes commands from `.Codex/.agentvibes-backup/` back to `.Codex/commands/agent-vibes/`
...
2. **Check if backup directory exists:**
   ```bash
   if [ ! -d ".Codex/.agentvibes-backup" ]; then
...
3. **Restore all command files:**
   ```bash
   mv .Codex/.agentvibes-backup/* .Codex/commands/agent-vibes/
   rmdir .Codex/.agentvibes-backup
   ```

4. **Remove the hidden state flag:**
   ```bash
   rm -f .Codex/.agentvibes-hidden.flag
   ```
...
5. **Display success message:**
   ...
   📂 All commands are now visible in `.Codex/commands/agent-vibes/`
