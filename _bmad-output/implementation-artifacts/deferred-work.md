...
## Deferred from: story 17-13-poi-search-overpass-user-agent-and-cache-coverage (2026-08-19)

- ~~**Aucune remontée UI quand Overpass tombe**~~ → **CORRIGÉ** le 2026-08-19 (story 17.14) : composant `ExtendedSearchStatus` — attente, lenteur au-delà de 5 s, et bandeau « résultats partiels » en cas d'échec. Description d'origine : : quand les 3 instances échouent, l'utilisateur ayant activé la « recherche étendue » voit uniquement les résultats Google Places, sans indication que la source complémentaire n'a pas répondu. C'est ce silence qui a laissé la panne Overpass invisible 5 mois. Piste : renvoyer un flag `sources: { overpass: 'ok' | 'unavailable' }` dans la réponse `/pois` et afficher un bandeau non bloquant (`<StatusBanner />`) côté planning et live. `apps/api/src/pois/pois.service.ts`
- ~~**Le toggle Overpass n'est pas un filtre d'affichage**~~ → **CORRIGÉ** le 2026-08-19 (story 17.13, T13) : `excludeSources` sur les deux chemins de lecture, OFF masque les POI `overpass` en cache. Mesuré : 66 POI en ON contre 16 en OFF sur la même fenêtre. Description d'origine : : les POI `source='overpass'` déjà en cache (TTL 30 j, vs 7 j pour Google) restent visibles après passage de l'option en OFF. Comportement d'origine, conservé en 17.13. À trancher : soit filtrer par source à la lecture quand `overpassEnabled=false`, soit documenter que l'option ne gouverne que la collecte.
- **Monitoring de la santé des sources externes** : aucune alerte sur « 0 insertion `source='overpass'` depuis N jours » ni sur un taux d'échec Overpass. Un check Uptime Kuma ou une métrique + seuil aurait détecté la régression en jours au lieu de mois. Vaut aussi pour Google Places (quota) et WeatherAPI.
- **`prefetchAndInsertGooglePois` : `inserted` non exploité** : la valeur de retour sert uniquement via `complete` ; `inserted` n'est que loggé. À utiliser si un jour on veut décider de re-tenter une bbox « couverte mais vide ». `apps/api/src/pois/pois.service.ts`
- ~~**Place Details séquentiels dans le prefetch**~~ → **CORRIGÉ** le 2026-08-19 (story 17.13, T14) : `mapWithConcurrency` (6 en vol) + insertion batchée par calque. Description d'origine : : la boucle `for (const placeId of placeIds)` enchaîne les appels Google un par un par layer (les 4 layers, eux, sont parallèles). Sur une bbox dense (jusqu'à 20 ids × 16 types), la latence de la première recherche reste élevée. Batcher par petits lots concurrents. `apps/api/src/pois/pois.service.ts`
- ~~**`amenity=shelter` pollue le calque « Refuge / Abri »**~~ → **CORRIGÉ** le 2026-08-19 dans la story 17.13 (T11) : `SLEEPABLE_SHELTER_TYPES` + `resolveCategory` aligné, 294 → 5 éléments sur la bbox de référence.
- **POI Overpass sans nom affichés « Unknown »** : `insertOverpassPois` retombe sur `'Unknown'` quand ni `name` ni `name:en` n'existent (237 abribus sur le seul segment de test). À trancher : filtrer les POI sans nom à l'insertion, ou afficher un libellé dérivé de la catégorie côté UI. `apps/api/src/pois/pois.repository.ts:57`
- ~~**Les pins POI ne sont pas peints avant la première interaction carte**~~ → **CORRIGÉ** le 2026-08-19 (story 17.13, T12) : `map.triggerRepaint()` en fin de construction des calques (planning + live) et après ajout d'images de pins. Description d'origine conservée pour mémoire : (observé le 2026-08-19 en validant la story 17.13, **reproductible**) : après une recherche, les compteurs de la sidebar sont corrects (Hôtel 51) mais **aucun pin ni cluster n'est dessiné** ; un seul cran de zoom (ou un pan) les fait apparaître **tous instantanément**. Donc données, source GeoJSON et calques sont bien en place — il manque un repaint après l'auto-zoom `fitToCorridorRange`. Cause probable : `registerPoiPinImages` est async (`void fn().then(() => addLayers())`) et se résout **après** la fin de l'animation de caméra, sans que rien ne redemande un rendu. Piste : `map.triggerRepaint()` après l'enregistrement des images / l'ajout des calques, ou enregistrer les images **avant** de lancer l'animation. Antérieur à la story 17.13 (l'API renvoie les bonnes données, vérifié en SQL). `apps/web/src/lib/poi-pin-factory.ts`, `apps/web/src/app/(app)/map/[id]/_components/hooks/use-poi-layers.ts`, auto-zoom dans `map-view.tsx`.
- **`staleTime: 30 jours` sur les queries POI + bouton « Rechercher » explicite = combinaison trompeuse** (relevé 2026-08-19 ; **arbitré non bloquant par Guillaume le 2026-08-20**, priorité basse) : `use-pois.ts` fixe `staleTime`/`gcTime` à `POI_BBOX_CACHE_TTL` (30 j). Re-cliquer « Rechercher » sur une fenêtre déjà interrogée n'émet aucune requête et réaffiche la réponse en cache. Observé pendant la validation de la 17.14 : un second clic a réaffiché 52 hôtels instantanément alors que la clé Redis Overpass venait d'être supprimée côté serveur.
  **Portée réelle, mesurée le 2026-08-20 — bien plus faible que ce que « 30 jours » suggère** : le web **ne persiste pas** son cache TanStack Query (`apps/web/src/lib/query-client.ts` = `QueryClient` en mémoire, aucun persister) → la rétention est bornée par la durée de l'onglet, un F5 suffit à repartir de zéro. Le mobile persiste, mais avec un `maxAge` de 24 h. Et dans l'usage courant, tout déplacement du slider change `fromKm`/`toKm`, donc la clé, donc émet une vraie requête. Le cas résiduel (recliquer sans rien changer) réaffiche des données **correctes** : c'est une malhonnêteté d'UI, pas une erreur de données. Le scénario qui l'a révélé — deux onglets, toggle changé dans l'un, retour dans l'autre — est le pire cas réaliste.
  Piste si repris : invalider `['pois']` au moment du `setSearchCommitted(true)` (coût serveur ~200 ms quand la bbox est déjà marquée côté Redis), en gardant un `gcTime` long pour la rétention hors-ligne. `apps/web/src/hooks/use-pois.ts`, `apps/web/src/app/(app)/map/[id]/_components/search-range-control.tsx`.
- **Jest mobile ne rend pas la main sur `src/hooks/use-live-poi-search.test.tsx`** : la suite passe en 0,7 s puis « Jest did not exit one second after the test run has completed » — un handle asynchrone n'est pas fermé (QueryClient / timer). Sans `--forceExit`, tout run ciblé sur ce fichier bloque jusqu'au timeout de l'appelant. À diagnostiquer avec `--detectOpenHandles`. Antérieur à la story 17.13.
