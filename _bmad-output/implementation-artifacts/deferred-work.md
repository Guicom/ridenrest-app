...
- **`staleTime: 30 jours` sur les queries POI + bouton « Rechercher » explicite = combinaison trompeuse** (relevé 2026-08-19, non corrigé — **nouvelle observation le même jour** : lors de la validation de la story 17.14, un second clic sur « Rechercher » a réaffiché 52 hôtels **instantanément et sans émettre la moindre requête**, alors que la clé Redis Overpass venait d'être supprimée côté serveur pour forcer un appel réel. Le geste utilisateur est donc totalement inopérant sur une fenêtre déjà interrogée.) : `use-pois.ts` fixe `staleTime`/`gcTime` à `POI_BBOX_CACHE_TTL` (30 j). Conséquence : re-cliquer « Rechercher » sur une fenêtre déjà interrogée **n'émet aucune requête** et réaffiche la réponse mise en cache, même si les données serveur ont changé depuis (nouveaux POI, ou changement de sémantique comme le filtre de source de la story 17.13). C'est ce qui a fait croire à un toggle sans effet pendant le diagnostic. Le geste utilisateur « Rechercher » est explicite : il devrait forcer un aller-retour. Piste : invalider `['pois']` au moment du `setSearchCommitted(true)` (coût serveur faible : ~200 ms quand la bbox est déjà marquée côté Redis), en gardant un `gcTime` long pour la rétention hors-ligne. `apps/web/src/hooks/use-pois.ts`, `apps/web/src/app/(app)/map/[id]/_components/search-range-control.tsx`.
