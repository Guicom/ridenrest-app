...
- **🔴 Finding BLOQUANT prod (2026-07-05)** : après le rebuild, login Google → « annulée ». Prod renvoie `{"code":"INVALID_CALLBACK_URL"}` sur `POST /api/auth/sign-in/social`. Root cause : **le backend mobile de `feature/mobile` n'a JAMAIS été déployé en prod.** `origin/main` (déployé) n'a ni `trustedOrigins:['ridenrest://',…]` ni le plugin `expo()` (introduits par `cd93308 MOB-2.1`, jamais mergé) → prod rejette le scheme mobile. Écart complet non déployé : auth mobile + **pages légales privacy/terms (MOB-6.4, requises AC2)** + module push (MOB-6.2) + **migration DB 0019 `push_tokens`**. `feature/mobile` = 76 commits devant `origin/main`, 0 derrière. **Décision Guillaume : déployer tout le backend mobile en prod** (merge → main). **Prérequis de déploiement corrigé** : `deploy.sh` faisait `turbo build` non filtré (aurait buildé `apps/mobile`/`expo export` sur le VPS sous `set -e` → risque OOM/abort) → scopé à `--filter=@ridenrest/web --filter=@ridenrest/api` (mobile = EAS, jamais VPS). ⇒ **L'AC1 (install+usage beta) est bloqué tant que ce déploiement prod n'est pas fait + vérifié** (migration 0019 + re-sonde `INVALID_CALLBACK_URL` disparu + retest login).
...
- `deploy.sh` (M) — `turbo build` VPS scopé `--filter=@ridenrest/web --filter=@ridenrest/api` (mobile buildé sur EAS, jamais sur le VPS).
- `apps/mobile/src/components/adventure/strava-import-sheet.test.tsx` (M) — fix flaky CI (test MOB-3.4 hors périmètre, mais bloquant le gate deploy) : `gcTime: 0` + `jest.setTimeout(15000)`. Suite mobile complète verte : 631/631.
...
| 2026-07-05 | 0.6 | **INCIDENT SÉCURITÉ prod (découvert pendant la prépa beta) — RÉSOLU.** Symptôme : recherche POI = 0 résultat en prod (OK local). RCA : **Redis prod exposé public SANS mot de passe** (`docker-compose` `6379:6379` en `0.0.0.0` ; idem Postgres `5432`) → attaque **`REPLICAOF 109.244.159.27:29846`** → Redis read-only replica → **BullMQ KO → parsing GPX bloqué → corridor vide → 0 POI** ; effet de bord : `ridenrest-api-error.log` **41 GB** (flood `READONLY`) → disque VPS quasi plein. **Pas de RCE** (`MODULE LIST` vide, cron/ssh/process propres — master pirate `down`). **Résolution** : `REPLICAOF NO ONE` + bind **`127.0.0.1`** db+redis (`docker-compose.yml` durci + `deploy.sh` réassert `up -d db redis` + VPS `docker compose up -d db redis` → `docker port` = 127.0.0.1 ✅) + `FLUSHALL` + `pm2 flush` (41 GB récupérés) + `pm2-logrotate`. **Recherche POI re-fonctionnelle en prod ✅.** Suivi non bloquant : roter `POSTGRES_PASSWORD` + `--requirepass` Redis. | claude-opus-4-8 |
| 2026-07-05 | 0.5 | **Deploy prod RÉUSSI** (run `28749146475`, PR #10) après 2 fixes CI (mobile strava flaky + web `e2e/**` exclu de Vitest). CI verte (web 1154 / mobile 631 / api 418 / packages) → « Deploy to VPS » success. Vérifs : migration `0019` appliquée (`push_tokens`), et **re-sonde `sign-in/social` → prod accepte `ridenrest://`** (renvoie l'URL Google, `redirect_uri https://ridenrest.app/api/auth/callback/google`) — `INVALID_CALLBACK_URL` résolu. Reste : retest login Google device (AC1). | claude-opus-4-8 |
| 2026-07-05 | 0.4 | **Déploiement backend mobile en prod (go Guillaume)** : PR #8 (feature/mobile→main) → CI a révélé 1 test flaky bloquant (`strava-import-sheet`, timeout 5 s en CI, handles/timers résiduels) → fix `gcTime:0` + `jest.setTimeout(15000)` (suite mobile 631/631 verte). PR #9 re-mergée pour relancer CI+deploy (run `28748830883`). deploy.sh scopé web+api. Vérifs post-deploy en cours (migration 0019 + re-sonde `INVALID_CALLBACK_URL` + retest login). | claude-opus-4-8 |
| 2026-07-05 | 0.3 | **Finding bloquant + prérequis déploiement prod** : smoke-test beta iOS → login Google KO. (1) build store sans URLs backend → défauts localhost → fix URLs prod en EAS env (rebuild). (2) BLOQUANT : backend mobile (`feature/mobile`, 76 commits) jamais déployé en prod → prod rejette `ridenrest://` (`INVALID_CALLBACK_URL`) + pages légales MOB-6.4 + push MOB-6.2 + migration 0019 absents. Décision : déployer tout le backend mobile (merge→main). Fix prérequis : `deploy.sh` scopé web+api (mobile≠VPS). Commits `7ed816a` (MOB-6.5) + `77a8a9c` (deploy.sh) poussés sur `feature/mobile`. **AC1 bloqué jusqu'au deploy prod + vérif.** | claude-opus-4-8 |
