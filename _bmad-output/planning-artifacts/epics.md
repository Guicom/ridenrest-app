...
**Given** une date et une heure de départ renseignées dans le formulaire,
**When** les étapes sont créées,
**Then** l'étape *i* porte `departureTime = première date + i jours` à la **même heure locale**, y compris à cheval sur un changement d'heure.
...
- **Dates de départ chaînées** : le formulaire porte une date + heure de départ ; l'étape *i* part à `première + i jours`, **à la même heure murale**. L'incrément doit être **calendaire** — `+86 400 000 ms` décale l'heure d'une heure au passage à l'heure d'hiver (départ 08:00 le 24 octobre → 07:00 le 25), et l'heure fausse part ensuite dans la prévision météo. Le client envoie donc `firstDepartureAt` + `timeZone` (IANA), et un helper pur `addDaysPreservingWallClock` de `packages/shared` fait l'arithmétique via `Intl` (aucune dépendance ajoutée). `timeZone` venant du client, un identifiant invalide *lève* un `RangeError` → validation + repli `'UTC'`. Conséquence voulue : les étapes générées ont ETA, météo par étape et déclenchent la synchro de date de fin de 17.12.
